...
class Producer:
    ...
    def user_registration(self, set_num):
        ...
        target = (
            f"{self.landing_zone}/registered_users_bz/1-registered_users_{set_num}.csv"
        )
    ...
    def _validate_count(self, format, location, expected_count):
        ...
        actual_count = (
            spark.read.format(format).option("header", "true").load(target).count()
        )
        assert (
            actual_count == expected_count
        ), f"Expected {expected_count:,} records, found {actual_count:,} in {location}"
