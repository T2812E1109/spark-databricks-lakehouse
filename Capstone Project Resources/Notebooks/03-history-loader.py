...
class HistoryLoader:
    ...
    def load_date_lookup(self):
        ...
        spark.sql(
            f"INSERT OVERWRITE TABLE {self.catalog}.{self.db_name}.date_lookup SELECT date, week, year, month, dayofweek, dayofmonth, dayofyear, week_part FROM json.`{self.test_data_dir}/6-date-lookup.json/`"
        )
        print("Done")
    ...
    def assert_count(self, table_name, expected_count):
        ...
        actual_count = spark.read.table(
            f"{self.catalog}.{self.db_name}.{table_name}"
        ).count()
        assert (
