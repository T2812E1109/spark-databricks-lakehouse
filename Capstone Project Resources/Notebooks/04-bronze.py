...
class Bronze:
    ...
    def consume_user_registration(self, once=True, processing_time="5 seconds"):
        ...
        if once == True:
            ...
            return stream_writer.trigger(processingTime=processing_time).toTable(
                f"{self.catalog}.{self.db_name}.registered_users_bz"
            )
    ...
    def consume_kafka_multiplex(self, once=True, processing_time="5 seconds"):
        ...
        df_date_lookup = spark.table(
            f"{self.catalog}.{self.db_name}.date_lookup"
        ...
        df_stream = (
            ...
            .join(
                ...
                [
                    F.to_date((F.col("timestamp") / 1000).cast("timestamp"))
                    == F.col("date")
                ],
                "left",
            )
    ...
    def assert_count(self, table_name, expected_count, filter="true"):
        ...
        actual_count = (
            spark.read.table(f"{self.catalog}.{self.db_name}.{table_name}")
            .where(filter)
            .count()
        )
        assert (
    ...
    def validate(self, sets):
        ...
        self.assert_count(
            "kafka_multiplex_bz", 7 if sets == 1 else 13, "topic='user_info'"
        )
        self.assert_count(
            "kafka_multiplex_bz", 16 if sets == 1 else 32, "topic='workout'"
        )
        self.assert_count("kafka_multiplex_bz", sets * 253801, "topic='bpm'")
        print(
            f"Bronze layer validation completed in {int(time.time()) - start} seconds"
        )
