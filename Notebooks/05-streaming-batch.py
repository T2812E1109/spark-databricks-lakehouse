...
class InvoiceStreamBatch:
    ...
    def append_invoices(self, flattenedDF: DataFrame, trigger="batch") -> DataFrame:
        writeStream = (
        ...
        if trigger == "batch":
            writeStream = writeStream.trigger(availableNow=True)
        else:
            writeStream = writeStream.trigger(processingTime=trigger)

        return writeStream.toTable("invoice_line_items")
    ...
    def process(self, trigger="batch"):
        ...
        flattenedDF = self.flatten_invoices(explodedDF)
        sQuery = self.append_invoices(flattenedDF, trigger)
        print("Done")
        return sQuery
