# MAGIC %run ./20-streaming-incremental-unbounded-aggregates
...
class AggregationTestSuite:
    ...
    def cleanTests(self):
        ...
        spark.sql(
            f"CREATE TABLE customer_rewards(CustomerCardNo STRING, TotalAmount DOUBLE, TotalPoints DOUBLE)"
        )
    ...
    def ingestData(self, itr):
        ...
        dbutils.fs.cp(
            f"{self.base_data_dir}/datasets/invoices/invoices_{itr}.json",
    ...
    def assertGold(self, expected_value):
        ...
        actual_value = spark.sql(
            "select TotalAmount from customer_rewards where CustomerCardNo = '2262471989'"
        ).collect()[0][0]
        assert (
            expected_value == actual_value
        ), f"Test failed! actual value is {actual_value}"
        print("Done")
