...
class medallionApproachTestSuite:
    ...
    def assertResult(self, expected_count):
        ...
        actual_count = spark.sql("select count(*) from invoice_line_items").collect()[
            0
        ][0]
        assert (
    ...
    def runTests(self):
        ...
        for f in dbutils.fs.ls(
            f"{self.base_data_dir}/data/invoices_archive/{self.base_data_dir}/data/invoices"
        ):
            assert (
                f.name in archives_expected
            ), f"Archive Validation failed for {f.name}"
        print("Done")
