...
class kafkaToBronzeTestSuite:
    ...
    def assertBronze(self, expected_count):
        ...
        ), f"Test failed! actual table count is {actual_count}"
        print("Done")
    ...
    def assertNotifications(self, start_time, expected_count):
        ...
        actual_count = (
            ...
            .option(
                ...
                f"{bz.JAAS_MODULE} required username='{bz.CLUSTER_API_KEY}' password='{bz.CLUSTER_API_SECRET}';",
            )
        ...
        assert (
            expected_count == actual_count
        ), f"Test failed! actual topic count is {actual_count}"
        print("Done")
    ...
    def runTests(self):
        ...
        spark.sql(
            f"CREATE TABLE invoices_bz (key STRING, value STRUCT<{value_schema}>, topic STRING, timestamp TIMESTAMP)"
        )
