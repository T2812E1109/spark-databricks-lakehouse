...
class Bronze:
    def __init__(self):
        ...
        self.BOOTSTRAP_SERVER = "pkc-abcdefgh.us-west4.gcp.confluent.cloud:9092"
    ...
    def sendToKafka(self, kafka_df):
        (
            kafka_df.writeStream.queryName("notification-feed")
            .format("kafka")
            .option("kafka.bootstrap.servers", self.BOOTSTRAP_SERVER)
            .option("kafka.security.protocol", "SASL_SSL")
            .option("kafka.sasl.mechanism", "PLAIN")
            ...
            .option(
                "checkpointLocation",
                f"{self.base_data_dir}/chekpoint/notification_feed",
            )
            .outputMode("append")
            .start()
        )
