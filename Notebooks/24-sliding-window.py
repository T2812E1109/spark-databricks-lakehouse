class SlidingAggregate:
    ...
    def getSensorData(self, kafka_df):
        ...
        return (
            kafka_df.select(
                kafka_df.key.cast("string").alias("SensorID"),
                from_json(kafka_df.value.cast("string"), self.getSchema()).alias(
                    "value"
                ),
            )
            .select("SensorID", "value.*")
            .withColumn(
                "CreatedTime", expr("to_timestamp(CreatedTime, 'yyyy-MM-dd HH:mm:ss')")
            )
        )
    ...
    def getAggregate(self, sensor_df):
        ...
        return (
            sensor_df.withWatermark("CreatedTime", "30 minutes")
            .groupBy(
                sensor_df.SensorID,
    ...
    def saveResults(self, results_df):
        ...
        return (
            results_df.writeStream.queryName("sensor-query")
            .option(
                "checkpointLocation", f"{self.base_data_dir}/chekpoint/sensor_summary"
            )
            .outputMode("complete")
